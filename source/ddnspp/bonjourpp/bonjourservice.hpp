...
namespace air{namespace bonjour {
	...
	//                                                                  
	************************************************************************/
	LocalService::LocalService(boost::asio::io_service& ios,const DNSDApi &dll,const LocalServiceEvtCallback& func)
	...
	CoreContextPtr		LocalService::getCoreContext()
	{
		return core;
	}

	LocalServiceEvtCallback				LocalService::setEvtCallback(const LocalServiceEvtCallback& func)
	{
		LocalServiceEvtCallback old=evtCallback;
		evtCallback=func;
		return old;
	}
	void        DNSSD_API LocalService::DNSServiceRegisterReplyCallback(
		DNSServiceRef                       sdRef,
		DNSServiceFlags                     flags,
		DNSServiceErrorType                 errorCode,
		const char                          *name,
