...
namespace air{namespace bonjour {
	...
	public:
		...
		static	uint16_t	MaxCharacterOfKeyName(){return 9;}
	...
	//                                                                  
	************************************************************************/
	TxtRecordEncoder::TxtRecordEncoder(const DNSDApi &dll,boost::uint16_t	maxBytes)
	...
	TxtRecordEncoder::~TxtRecordEncoder()
	{
		dnsDll.getFunctiontable().funcTXTRecordDeallocate(&recordHolder);
	}

	BonjourError	TxtRecordEncoder::putOrReplace(const air::common::StringMap& strMap)
	{
		BonjourError err;
	...
	BonjourError	TxtRecordEncoder::putOrReplace(
		const std::string&       key,
		boost::uint8_t   valueBytes,
		const void       *value )
	{
	...
	void			TxtRecordEncoder::remove(const std::string&       key)
	{
		dnsDll.getFunctiontable().funcTXTRecordRemoveValue(&recordHolder,key.c_str());
	}
	const char*		TxtRecordEncoder::getRecordPtr()const
	{
		return (const char*)dnsDll.getFunctiontable().funcTXTRecordGetBytesPtr(&recordHolder);
	}
	std::string		TxtRecordEncoder::getRecordAsString()const
	{
		return std::string(getRecordPtr(),getRecordLenth());
	}
	boost::uint16_t	TxtRecordEncoder::getRecordLenth()const
	{
	...
	ConstBuffer::ConstBuffer(const void* data,boost::uint16_t len)
		:ptr(data),length(len)
	{
		//
	}
	boost::uint16_t	ConstBuffer::getLength()const
	{
		return length;
	}
	const void*		ConstBuffer::getPtr()const
	{
		return ptr;
	}
	const unsigned char*		ConstBuffer::getUcharPtr()const
	{
		return (const unsigned char*)ptr;
	}
	bool			ConstBuffer::empty()const
	{
		return (length == 0 || ptr == NULL);
	}
	std::string		ConstBuffer::asString(boost::uint16_t offset,boost::uint16_t len)const
	{
		BOOST_ASSERT( len + offset <= length);
		return std::string((const char*)ptr + offset,len);
	}
	std::string		ConstBuffer::allAsString()const
	{
		return asString(0,length);
	}

	//////////////////////////////////////////////////////////////////////////
	TxtRecordDecoder::VALUE_FILED_TYPE	TxtRecordDecoder::getValueType(const ConstBuffer& val)
	{
	...
	TxtRecordDecoder::TxtRecordDecoder(const DNSDApi &dll,const void* data,boost::uint16_t len)
	...
	BonjourError		TxtRecordDecoder::getAll(ConstRecordList& list)const
	{
	...
	void				TxtRecordDecoder::print(const ConstRecordList& list,std::ostream& os)
	{
	...
	void				TxtRecordDecoder::praseThenPrint(std::ostream& os)
	{
		ConstRecordList all;
		getAll(all);
		print(all,os);
