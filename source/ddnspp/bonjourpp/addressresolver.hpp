...
namespace air{namespace bonjour {
	...
	************************************************************************/
	...
	AddressResolver::AddressResolver(boost::asio::io_service& ios,const DNSDApi &dll,const AddressResolverEvtCallback& func)
	...
	CoreContextPtr		AddressResolver::getCoreContext()
	{
		return core;
	}

	AddressResolverEvtCallback	AddressResolver::setEvtCallback(const AddressResolverEvtCallback& func)
	{
		AddressResolverEvtCallback old=evtCallback;
		evtCallback=func;
		return old;
	}
	void		DNSSD_API	AddressResolver::DNSServiceGetAddrInfoReplyCallback(
		DNSServiceRef                    sdRef,
		DNSServiceFlags                  flags,
		uint32_t                         interfaceIndex,
		DNSServiceErrorType              errorCode,
	...
	void	AddressResolver::evtHanler(
		DNSServiceRef                    sdRef,
		DNSServiceFlags                  flags,
		uint32_t                         interfaceIndex,
		DNSServiceErrorType              errorCode,
