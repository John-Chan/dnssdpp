...
namespace air{namespace bonjour {
	...
	//                                                                  
	************************************************************************/
	CoreContext::CoreContext(boost::asio::io_service& ios,const DNSDApi& dll,bool	strandExecHandler)
	...
	boost::asio::io_service& 	CoreContext::getIOService()
	{
		return strand.get_io_service();
	}
	const DNSDApi&		CoreContext::getApiProvider()const
	{
		return dnsDll;
	}
	DNSServiceRef&   CoreContext::getDNSServiceRef()
	{
		return serviceRef;
	}
	void	CoreContext::startEventLoop(const boost::weak_ptr<void>&	evtProcesser)
	{
		if(socketFD == -1){
			obPtr=evtProcesser;
			stoped=false;
	...
	void	CoreContext::handle_read(boost::system::error_code ec)
	{
		reading=false;
		if(ec){
			if(stoped){
	...
	bool	CoreContext::process_read_data()
	{
